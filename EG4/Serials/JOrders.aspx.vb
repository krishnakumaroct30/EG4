        ...
        Try
            If Session.Item("IsThisU") <> Nothing Then
                ...
                Else
                    ...
                    CreateSUserButton = Master.FindControl("Accordion1").FindControl("SerPane").FindControl("Ser_Order_Bttn")
...
System.Web.Services.WebMethod()> _
    ...
    Protected Sub DDL_Approvals_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles DDL_Approvals.SelectedIndexChanged
        ...
        Try
            ...
            If DDL_Approvals.Text <> "" Then
                ...
                SQL = "SELECT APP_DATE, COM_CODE  FROM ACQUISITIONS WHERE (APP_NO = '" & Trim(APP_NO) & "') AND  (LIB_CODE ='" & (LibCode) & "') AND (SUBS_YEAR IS NOT NULL)"
    ...
    Public Sub PopulateAppGrid(ByVal APP_NO As Object)
        ...
        Try
            If APP_NO <> "" Then
                Dim SQL As String = Nothing
                SQL = "SELECT ACQ_ID, APP_NO, APP_DATE, VOL_NO, PROCESS_STATUS, COPY_PROPOSED, COPY_APPROVED, COPY_ORDERED, COM_CODE, TITLE, ORDER_NO, ORDER_DATE, VEND_NAME, SUBS_YEAR FROM APPROVAL_VIEW2 WHERE (APP_NO = '" & APP_NO & "')  AND (PROCESS_STATUS = 'Approved') AND (LIB_CODE ='" & (LibCode) & "') AND  (BIB_CODE = 'S') ORDER BY ACQ_ID DESC"
...
System.Web.Services.WebMethod()> _
    ...
    Public Sub chckchkbox()
        For Each row As GridViewRow In Grid3.Rows
            If row.RowType = DataControlRowType.DataRow Then
                ...
                txtsn = DirectCast(row.FindControl("txt_App_CopyOrd"), TextBox)
                If cb IsNot Nothing AndAlso cb.Checked = True Then
                    txtsn.Enabled = True
                Else
                    txtsn.Enabled = False
    ...
    Public Sub PopulateOrderGrid(ByVal ORDER_NO As Object)
        ...
        Try
            If ORDER_NO <> "" Then
                Dim SQL As String = Nothing
                SQL = "SELECT ACQ_ID, ORDER_NO, ORDER_DATE, SUBS_YEAR, PROCESS_STATUS, COPY_PROPOSED, COPY_APPROVED, COPY_ORDERED, VEND_NAME, TITLE, SUB_TITLE, EDITOR, PLACE_OF_PUB, PUB_PLACE, STANDARD_NO, PUB_NAME, CAT_NO, CORPORATE_AUTHOR FROM APPROVAL_VIEW2 WHERE (ORDER_NO = '" & ORDER_NO & "')  AND (PROCESS_STATUS = 'Approved' OR PROCESS_STATUS = 'Ordered') AND (LIB_CODE ='" & (LibCode) & "') AND  (BIB_CODE = 'S') ORDER BY ACQ_ID DESC"
    ...
    Public Function Report_Load_Letter() As ReportDocument
        ...
        Try
            ...
            dt = ViewState("dt")
            If dt.Rows.Count <> 0 Then
                Rpt.Load(Server.MapPath("~/Reports/Journals_Order_Report.rpt"))
