...
Public Class Approvals
        ...
        Try
            If Session.Item("IsThisU") <> Nothing Then
                ...
                Else
                    ...
                    CreateSUserButton = Master.FindControl("Accordion1").FindControl("AcqPane").FindControl("Acq_AddApproval_Bttn")
    ...
    Public Sub SearchTitle()
        ...
        Try
            ...
            Else
                SQL = "select  CAT_NO, TITLE from CATS WHERE (CAT_NO IS NOT NULL) AND (BIB_CODE <>'S') ORDER BY CAT_NO DESC "
            End If
                ...
                If dt.Rows.Count <> 0 Then
                    ...
                    If dt.Rows(0).Item("MULTI_VOL").ToString <> "" Then
                        ...
                        If dt.Rows(0).Item("MULTI_VOL").ToString = "Y" Then
                            txt_Acq_VolNo.Enabled = True
                        Else
                            txt_Acq_VolNo.Enabled = False
                        End If
                    Else
                        Label23.Text = ""
                        txt_Acq_VolNo.Enabled = False
...
System.Web.Services.WebMethod()> _
    ...
    Protected Sub App_Save_Bttn_Click(ByVal sender As Object, ByVal e As EventArgs) Handles App_Save_Bttn.Click
        If IsPostBack = True Then
            ...
            Try
                ...
                'validation for Vol No
                Dim VOL_NO As Object = Nothing
                If Label23.Text = "Y" Then
                    If Me.txt_Acq_VolNo.Text <> "" Then
                        VOL_NO = TrimAll(txt_Acq_VolNo.Text)
                        VOL_NO = RemoveQuotes(VOL_NO)
                        If VOL_NO.Length > 20 Then
                            Label6.Text = "Error: Input is not Valid !"
                            Label15.Text = ""
                            txt_Acq_VolNo.Focus()
                            Exit Sub
                ...
                objCommand.CommandText = "INSERT INTO ACQUISITIONS (CAT_NO, LIB_CODE, APP_NO, VOL_NO, COPY_PROPOSED, CUR_CODE, ITEM_PRICE, RECOMMENDED_BY, RECOMMENDED_DATE, NOTE, PROCESS_STATUS, DATE_ADDED, USER_CODE, ACQMODE_CODE,IP) " & _
                                         " VALUES (@CAT_NO, @LIB_CODE, @APP_NO, @VOL_NO, @COPY_PROPOSED, @CUR_CODE, @ITEM_PRICE, @RECOMMENDED_BY, @RECOMMENDED_DATE, @NOTE, @PROCESS_STATUS, @DATE_ADDED, @USER_CODE, @ACQMODE_CODE,@IP);  " & _
                ...
                If RECOMMENDED_BY = "" Then RECOMMENDED_BY = System.DBNull.Value
                objCommand.Parameters.Add("@RECOMMENDED_BY", SqlDbType.NVarChar)
                objCommand.Parameters("@RECOMMENDED_BY").Value = RECOMMENDED_BY

                If RECOMMENDED_DATE = "" Then RECOMMENDED_DATE = System.DBNull.Value
                ...
                If NOTE = "" Then NOTE = System.DBNull.Value
    ...
    Public Sub PopulateGrid()
        ...
        Try
            ...
            If DDL_Titles.Text <> "" Then
                Dim SQL As String = Nothing
                SQL = "SELECT ACQ_ID, APP_NO, APP_DATE, VOL_NO, PROCESS_STATUS, COPY_PROPOSED, CUR_CODE, ITEM_PRICE FROM ACQUISITIONS WHERE (CAT_NO IS NOT NULL) AND (CAT_NO = '" & Trim(Label19.Text) & "') AND (LIB_CODE = '" & Trim(LibCode) & "')  AND (PROCESS_STATUS = 'Requested' OR PROCESS_STATUS = 'Rejected') ORDER BY ACQ_ID DESC"
                ...
                If Not String.IsNullOrEmpty(AcqID) And AcqID <> 0 Then
                    ...
                    If dr.HasRows = True Then
                        ...
                        If dr.Item("VOL_NO").ToString <> "" Then
                            txt_Acq_VolNo.Text = dr.Item("VOL_NO").ToString
                        Else
                            txt_Acq_VolNo.Text = ""
                        End If
                ...
                If Label4.Text <> "" Then
                    ...
                    If ds.Tables("ACQ").Rows.Count <> 0 Then
                        ...
                        If Not String.IsNullOrEmpty(VOL_NO) Then
                            ds.Tables("ACQ").Rows(0)("VOL_NO") = VOL_NO.Trim
                        Else
                            ds.Tables("ACQ").Rows(0)("VOL_NO") = System.DBNull.Value
                        End If
    ...
    Public Sub PopulateAppNo()
        ...
        Me.DDL_Approvals.DataSource = GetApprovalList2(LibCode)
    ...
    Public Sub PopulateAppGrid(ByVal APP_NO As Object)
        ...
        Try
            If APP_NO <> "" Then
                '**********************************************************************************
                Dim SQL As String = Nothing
                SQL = "SELECT ACQ_ID, APP_NO, APP_DATE, VOL_NO, PROCESS_STATUS, COPY_PROPOSED, CUR_CODE, ITEM_PRICE, CAT_NO, COM_CODE, TITLE, SUB_TITLE, AUTHOR1, AUTHOR2, AUTHOR3, EDITION, PLACE_OF_PUB, YEAR_OF_PUB,PUB_PLACE, STANDARD_NO, PUB_NAME FROM APPROVAL_VIEW2 WHERE (APP_NO = '" & APP_NO & "')  AND (PROCESS_STATUS = 'Requested' or PROCESS_STATUS = 'Sent For Approval') AND (LIB_CODE ='" & (LibCode) & "') AND  (BIB_CODE <> 'S') ORDER BY ACQ_ID DESC; "
    ...
    Protected Sub DDL_Approval3_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles DDL_Approval3.SelectedIndexChanged
        ...
        Try
            ...
            If DDL_Approval3.Text <> "" Then
                ...
                Dim SQL As String = Nothing
                SQL = "SELECT ACQ_ID, APP_NO, APP_DATE, COM_CODE, PROCESS_STATUS, COPY_PROPOSED, COPY_APPROVED  FROM ACQUISITIONS WHERE (APP_NO = '" & Trim(APP_NO) & "') AND (LIB_CODE ='" & (LibCode) & "')  "
                Dim ds As New DataSet
                Dim da As New SqlDataAdapter(SQL, SqlConn)
                If SqlConn.State = 0 Then
                    SqlConn.Open()
    ...
    Public Sub PopulateAppGrid3(ByVal APP_NO As Object)
        ...
        Try
            If APP_NO <> "" Then
                '**********************************************************************************
                Dim SQL As String = Nothing
                SQL = "SELECT ACQ_ID, APP_NO, APP_DATE, VOL_NO, PROCESS_STATUS, COPY_PROPOSED, COPY_APPROVED, CUR_CODE, ITEM_PRICE, CAT_NO, COM_CODE, TITLE FROM APPROVAL_VIEW2 WHERE (APP_NO = '" & APP_NO & "')  AND (PROCESS_STATUS = 'Approved' or PROCESS_STATUS = 'Sent For Approval' OR PROCESS_STATUS = 'Rejected') AND (LIB_CODE ='" & (LibCode) & "') AND  (BIB_CODE <> 'S') ORDER BY ACQ_ID DESC"
    ...
    Public Function Report_Load_Letter() As ReportDocument
        ...
        Try
            ...
            If dt.Rows.Count <> 0 Then
                ...
                Rpt.Load(Server.MapPath("~/Reports/Books_Approval_Report.rpt"))
